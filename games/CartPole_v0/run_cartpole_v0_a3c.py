...
OUTPUT_GRAPH = True
LOG_DIR = './logs'
N_WORKERS = multiprocessing.cpu_count()  # That's the only usage of multiprocessing.
MAX_GLOBAL_EP = 1000
    ...
    def __init__(self, scope, globalAC=None):
        ...
        if scope == GLOBAL_NET_SCOPE:   # get global network
            ...
            with tf.variable_scope(scope):
                ...
                self.a_prob, self.v, self.a_params, self.c_params = self._build_net(scope)  # self.v is eval v.
...
class Worker(object):
    def __init__(self, name, globalAC):
        self.env = gym.make(GAME).unwrapped  # every agent has its own copy of the environment.
        self.name = name
